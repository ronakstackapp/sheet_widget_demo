import 'package:sheet_widget_demo/sqflite_demo/global/constant/db_constant.dart';
import 'package:sqflite/sqflite.dart';
import '../user_bean.dart';
import 'db_helper.dart';

class UserTable {
  static final List<String> sqLitew = UserBean.empty().toMap().keys.toList();
  ...
  static Future<List> checkUserExist(String emailId) async {
    ...
    var results = await dbClient.rawQuery(
        "SELECT * FROM ${DbConstant.LOGIN_TABLE} WHERE $USER_EMAILID " + " = '" + emailId + "'",
        null);
  ...
  static Future<List<UserBean>> getUserAllInfo() async {
    ...
    return userInfo.map((itemWord) => UserBean.map(itemWord)).toList();
  }
...
//  Get all users specific info which make process fast
  static Future<List> getSelectedUserField() async {
    Database dbClient = await dbHelper.database;
    var result =
        await dbClient.query(DbConstant.LOGIN_TABLE, columns: [USER_EMAILID, USER_PASSWORD]);
    return result.toList();
  }

  static Future<int> userCount() async {
    Database dbClient = await dbHelper.database;
    return Sqflite.firstIntValue(await dbClient.rawQuery('SELECT COUNT(*) FROM $USER_LOGIN_TABLE'));
  }
  ...
  static Future<int> deleteUser(String emailId) async {
    ...
    return await dbClient
        .delete(USER_LOGIN_TABLE, where: '$USER_EMAILID = ?', whereArgs: [emailId]);
  }
